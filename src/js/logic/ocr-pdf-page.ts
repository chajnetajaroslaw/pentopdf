...
const OCR_LANG_STORAGE_KEY = 'bentopdf:ocrPdf:selectedLangs';

function getPreferredTesseractLanguage(): string {
  const urlLang = getLanguageFromUrl();

  if (urlLang === 'pl') return 'pol';
  if (urlLang === 'de') return 'deu';
  if (urlLang === 'tr') return 'tur';
  if (urlLang === 'it') return 'ita';
  if (urlLang === 'id') return 'ind';
  if (urlLang === 'vi') return 'vie';
  if (urlLang === 'zh') {
    const browserLang = (navigator.language || '').toLowerCase();
    const isTraditional = /(tw|hk|mo)/.test(browserLang);
    return isTraditional ? 'chi_tra' : 'chi_sim';
  }
  ...
  return localeMap[base] || 'eng';
}

function readStoredSelectedLanguageCodes(): string[] {
  try {
...
function storeSelectedLanguageCodes(codes: string[]) {
  try {
    localStorage.setItem(OCR_LANG_STORAGE_KEY, JSON.stringify(codes));
  } catch {
    // ignore
  }
}
...
async function runOCR() {
  ...
  if (selectedLangs.length === 0) {
    showAlert(
      t('tools:ocrPdf.alerts.noLanguagesSelectedTitle'),
      t('tools:ocrPdf.alerts.noLanguagesSelectedMessage')
    );
    return;
  }
  ...
  if (!pageState.file) {
    showAlert(
      t('tools:ocrPdf.alerts.noFileTitle'),
      t('tools:ocrPdf.alerts.noFileMessage')
    );
    return;
  }
  ...
  try {
    ...
    try {
      ...
      latinFont = primaryFont;
      showAlert(
        t('tools:ocrPdf.alerts.fontWarningTitle'),
        t('tools:ocrPdf.alerts.fontWarningMessage')
      );
    }
      ...
      if (data.hocr) {
        ...
        words.forEach(function (word: OcrWord) {
          ...
          try {
            ...
            console.warn(
              `Could not calculate text width for "${text}":`,
              error
            );
            return;
          }
    ...
    console.error(e);
    showAlert(
      t('tools:ocrPdf.alerts.ocrErrorTitle'),
      t('tools:ocrPdf.alerts.ocrErrorMessage')
    );
...
function updateSelectedLanguagesUI() {
  ...
  const selectedNames = selectedCodes
    .map(function (code) {
      return tesseractLanguages[code as keyof typeof tesseractLanguages];
    })
    .filter(Boolean);
  ...
  if (selectedLangsDisplay) {
    selectedLangsDisplay.textContent =
      selectedNames.length > 0
        ? selectedNames.join(', ')
        : t('tools:ocrPdf.languages.none');
  }

  if (processBtn) {
    processBtn.disabled = !(pageState.file && selectedCodes.length > 0);
  }

  storeSelectedLanguageCodes(selectedCodes);
}
...
function restoreSelectedLanguages() {
  ...
  const stored = readStoredSelectedLanguageCodes();
  const preferred = getPreferredTesseractLanguage();
  const desired = stored.length > 0 ? stored : [preferred];

  const checkboxes = langList.querySelectorAll(
    '.lang-checkbox'
  ) as NodeListOf<HTMLInputElement>;

  checkboxes.forEach(function (cb) {
    cb.checked = desired.includes(cb.value);
  });
}
